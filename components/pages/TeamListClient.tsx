'use client'

import { motion } from 'framer-motion'
import Image from 'next/image'
import { urlFor } from '@/lib/sanity/client'
import type { SanityImageSource } from '@sanity/image-url/lib/types/types'

interface TeamMember {
  _id: string
  name: string
  role: string
  department?: string
  photo?: SanityImageSource
  bio?: string
  email?: string
}

interface TeamListClientProps {
  teamMembers: TeamMember[]
}

export function TeamListClient({ teamMembers }: TeamListClientProps) {
  // Group by department if available
  const groupedByDepartment = teamMembers.reduce((acc, member) => {
    const dept = member.department || 'General'
    if (!acc[dept]) acc[dept] = []
    acc[dept].push(member)
    return acc
  }, {} as Record<string, TeamMember[]>)

  return (
    <section className="section-padding bg-white">
      <div className="container mx-auto container-padding">
        <div className="max-w-7xl mx-auto">
          {Object.keys(groupedByDepartment).map((department, deptIndex) => (
            <div key={department} className="mb-16">
              {Object.keys(groupedByDepartment).length > 1 && (
                <h2 className="text-3xl font-bold text-slate-900 mb-8">{department}</h2>
              )}
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {groupedByDepartment[department].map((member, index) => (
                  <motion.div
                    key={member._id}
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.6, delay: (deptIndex * 0.1) + (index * 0.1) }}
                    className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all border border-gray-100 text-center"
                  >
                    {member.photo ? (
                      <div className="relative w-32 h-32 mx-auto mb-6 rounded-full overflow-hidden">
                        <Image
                          src={urlFor(member.photo).width(300).height(300).auto('format').url()}
                          alt={member.name}
                          fill
                          className="object-cover"
                        />
                      </div>
                    ) : (
                      <div className="w-32 h-32 mx-auto mb-6 rounded-full bg-gradient-to-br from-[#0B334A] to-[#0F4A6A] flex items-center justify-center">
                        <span className="text-5xl text-white">
                          {member.name.charAt(0).toUpperCase()}
                        </span>
                      </div>
                    )}
                    <h3 className="text-2xl font-bold text-slate-900 mb-2">{member.name}</h3>
                    <p className="text-[#0B334A] font-semibold mb-4">{member.role}</p>
                    {member.bio && (
                      <p className="text-slate-600 text-sm mb-4">{member.bio}</p>
                    )}
                    {member.email && (
                      <a
                        href={`mailto:${member.email}`}
                        className="text-sm text-[#0B334A] hover:underline"
                      >
                        {member.email}
                      </a>
                    )}
                  </motion.div>
                ))}
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  )
}

